...
Install NuGet:   https://www.nuget.org/packages/socket.framework  
Package Manager: Install-Package socket.framework    
.Net CLI: dotnet add package socket.framework    
Paket CLI: paket add socket.framework   
...
他是一个基于C＃framework4.0的套接字框架，使用异步连接，异步发送，异步接收，性能突发和压力测试。
---
安装NuGet:  https://www.nuget.org/packages/socket.framework    
Package Manager: Install-Package socket.framework   
.Net CLI :dotnet add package socket.framework      
Paket CLI:paket add socket.framework     
